        ...
        if messages:  # If there are any messages, iterate else sys.exit()
            ...
            for msg in messages:  # main iteration
                ...
                try:
                    ...
                    if (
                        "Delete Income Report#" in subject
                    ...
                    subject = subject.replace("Delete Income Report#", "")
                    ...
                    gsheets_range = f"{gsheets_tab}!C1:C15" # This range in Column C has the names to search within
                    ...
                    gsheets_range = (
                        f"{gsheets_tab}!A{row_with_name}:D{row_with_name}"
                    )
                    ...
                    # Clear the info
                    serviceSheets.spreadsheets().values().clear(
                        spreadsheetId=gsheets_id,
                        range=gsheets_range,
                        body=clear_values_request_body,
                    ).execute()

                    # Get the current sheet's ID (gid) because we need it for a batchUpdate
                    spreadsheet = (
                        serviceSheets.spreadsheets()
                    ...
                    # Get info ready to sort and resize columns
                    requests = [
                        {
                            "sortRange": {
                                "range": {
